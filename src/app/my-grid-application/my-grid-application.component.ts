    ...
    get testRows(): any {
        return this.test ? this.test.results.map((result) => {
            return {
                ...
                "task1": { numCorrect: result.task1, eval: this.testParams["task1"].evals.find(range => range.from <= result.task1 && range.to >= result.task1) },
                "task2a": { numCorrect: result.task2a, eval: this.testParams["task2a"].evals.find(range => range.from <= result.task2a && range.to >= result.task2a) },
                "task2b": { numCorrect: result.task2b, eval: this.testParams["task2b"].evals.find(range => range.from <= result.task2b && range.to >= result.task2b) }
            };
        }) : [];
    }
